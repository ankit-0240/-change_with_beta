...
quarterly_stock_prices = data.resample('Q').last()
quarterly_market_prices = market_data.resample('Q').last()


quarterly_stock_returns = quarterly_stock_prices.pct_change(fill_method=None) * 100
quarterly_market_returns = quarterly_market_prices.pct_change(fill_method=None) * 100
...
for ticker in stock_tickers:
    if ticker != market_ticker:
        stock_returns = quarterly_stock_returns[ticker].dropna()
        if not stock_returns.empty:
            for date in stock_returns.index:
                if date in quarterly_market_returns.index:
                    stock_return_value = stock_returns.loc[date]
                    market_return_value = quarterly_market_returns.loc[date]
                    
                    
                    covariance = (stock_return_value - stock_returns.mean()) * (market_return_value - quarterly_market_returns.mean())
                    variance = quarterly_market_returns.var()
                    
                    
                    beta = covariance / variance if variance != 0 else np.nan
                    result_data.append({'Datetime': date, 'Ticker': ticker, 'Quarterly_Percent_Change': stock_return_value, 'Beta': beta})
...
if not result_df.empty:
    result_df.to_excel('stock_quarterly_changes_with_beta.xlsx', index=False)
    print("Data saved to stock_quarterly_changes_with_beta.xlsx")
else:
