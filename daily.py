...
daily_stock_returns = data.pct_change(fill_method=None) * 100
daily_market_returns = market_data.pct_change(fill_method=None) * 100


daily_stock_returns.index = daily_stock_returns.index.tz_localize(None)
daily_market_returns.index = daily_market_returns.index.tz_localize(None)
...
for ticker in stock_tickers:
    if ticker != market_ticker:
        stock_returns = daily_stock_returns[ticker].dropna()
        if not stock_returns.empty:
            for date in stock_returns.index:
                if date in daily_market_returns.index:
                    stock_return_value = stock_returns.loc[date]
                    market_return_value = daily_market_returns.loc[date]
                    
                    
                    covariance = (stock_return_value - stock_returns.mean()) * (market_return_value - daily_market_returns.mean())
                    variance = daily_market_returns.var()
                
                    
                    beta = covariance / variance if variance != 0 else np.nan
                    result_data.append({'Datetime': date, 'Ticker': ticker, 'Daily_Percent_Change': stock_return_value, 'Beta': beta})
...
if not result_df.empty:
    result_df.to_excel('stock_daily_changes_with_beta.xlsx', index=False)
    print("Data saved to stock_daily_changes_with_beta.xlsx")
else:
