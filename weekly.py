...
weekly_stock_data = data.resample('W-FRI').last()
weekly_market_data = market_data.resample('W-FRI').last()

weekly_stock_returns = weekly_stock_data.pct_change(fill_method=None) * 100
weekly_market_returns = weekly_market_data.pct_change(fill_method=None) * 100

weekly_stock_returns.index = weekly_stock_returns.index.tz_localize(None)
weekly_market_returns.index = weekly_market_returns.index.tz_localize(None)
...
for ticker in stock_tickers:
    if ticker != market_ticker:
        stock_returns = weekly_stock_returns[ticker].dropna()
        if not stock_returns.empty:
            for date in stock_returns.index:
                if date in weekly_market_returns.index:
                    stock_return_value = stock_returns.loc[date]
                    market_return_value = weekly_market_returns.loc[date]
                    
                    
                    covariance = (stock_return_value - stock_returns.mean()) * (market_return_value - weekly_market_returns.mean())
                    variance = weekly_market_returns.var()
                    
                    
                    beta = covariance / variance if variance != 0 else np.nan
                    result_data.append({'Datetime': date, 'Ticker': ticker, 'Weekly_Percent_Change': stock_return_value, 'Beta': beta})
...
if not result_df.empty:
    result_df.to_excel('stock_weekly_changes_with_beta.xlsx', index=False)
    print("Data saved to stock_weekly_changes_with_beta.xlsx")
else:
